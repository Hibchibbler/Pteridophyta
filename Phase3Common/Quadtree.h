...
public:
    ...
    std::vector<XY> search(AABB searchRect)
    {
        std::vector<XY> points;
     ...
     bool search(Node *node, AABB searchRect, std::vector<XY> &points)
    {
        if (node == nullptr)
            return false;
    ...
    bool insert(Node **node, XY point)
    {
